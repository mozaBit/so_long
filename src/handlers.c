...
/*   Updated: 2023/11/13 18:50:18 by bmetehri         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	if (key == KEY_SW || key == KEY_CW
		|| key == KEY_UP)
		player_move(game, game->map.character.y - 1, \
			game->map.character.x, BACK);
	if (key == KEY_SS || key == KEY_CS
		|| key == KEY_DOWN)
		player_move(game, game->map.character.y + 1, \
			game->map.character.x, FRONT);
	if (key == KEY_SD || key == KEY_CD
		|| key == KEY_RIGHT)
		player_move(game, game->map.character.y, \
			game->map.character.x + 1, RIGHT);
	if (key == KEY_SA || key == KEY_CA
		|| key == KEY_LEFT)
		player_move(game, game->map.character.y, \
			game->map.character.x - 1, LEFT);
	if (key == KEY_CQ || key == KEY_SQ
		|| key == KEY_ESC)
		close_game(game);
	return (0);
}
